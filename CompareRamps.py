...
def FF(x, a, b, c, d, e, f, g):
    q1 = (2*a + 1)/(2*a)
    q2 = (2*d + 1)/(2*d)
    polyP = b*(q1/np.tanh(q1*c*x) + 1/(np.tanh(c*x/a)*a))
    polyP += e*(q2/np.tanh(q2*f*x) + 1/(np.tanh(d*x/a)/d))
    polyP += g*x**2
    return polyP

ff = FF(Bdata, -5.67252155e-01, -5.70154333e+00,  8.55624761e+00,  3.02469772e-01,  4.55618878e+00,  7.17531028e-01, 0)
p0 = [-5.67252155e-01, -5.70154333e+00,  8.55624761e+00,  3.02469772e-01,  4.55618878e+00,  7.17531028e-01, 0]
popt, pcov = curve_fit(FF, Bdata, fdata/Bdata, p0=p0, maxfev=5000)
ff = FF(Bdata, popt[0],popt[1], popt[2], popt[3], popt[4], popt[5], popt[6])
print(popt)
print(np.sqrt(np.diag(pcov)))

plt.plot(Bdata, ff, color = 'black', label = 'fit')
plt.plot(Bdata, fdata/Bdata, color = 'red', label = 'data')
plt.ylabel(r'f$_{0}$ (Hz)')
plt.xlabel('B (T)')
plt.legend(loc = 'best')
plt.show()
